...
## 数组相关
### 数组的初始化函数
memset(数组名,初始化的值,初始化的范围(eg.sizeof(数组名))
### 整数二分
>考虑好边界情况
```c
class Solution {
...
```
二维形式
<img width="2128" height="220" alt="图片" src="https://github.com/user-attachments/assets/494c1f7f-6aee-41b2-97de-3f9e33959359" />
```c
#include <vector>
...
```
>1.​引用传递(&)​​：高效传递大矩阵，避免拷贝开销
>2.​独立preSum​：新建专用数据结构存储前缀和，与原始矩阵分离
>3.​空间换时间​：一次O(mn)预处理，换取每次O(1)查询
>4.​安全设计​：原始矩阵保持不变，preSum独立计算
### 差分数组
```c
// 差分数组工具类
class Difference {
